...
|--------|----------------------------|----------------------------------------------------|
| POST   | `/api/v1/xml/upload`       | Upload one or more XML files                       |
| GET    | `/api/v1/nfe`              | List all processed NF-es (excludes errors)         |
...
curl -X POST http://localhost:8080/api/v1/xml/upload \
  -F "files=@exemplos/nfe_compra.xml" \
  -F "files=@exemplos/nfe_venda.xml" \
  -F "files=@exemplos/nfe_nao_identificada.xml"
...
│   │   └── api/
│   │       ├── handler/                # HTTP handlers (Gin) + generic Response[T]
│   │       └── middleware/             # Request logger
│   └── docs/                           # Generated by swag
├── frontend/
│   └── src/
│       ├── pages/                      # UploadPage, DashboardPage, UnidentifiedPage, QuarantinePage
│       ├── components/                 # StatCard, OperationBadge, StatusBadge
│       ├── services/api.ts             # API communication layer
│       ├── types/index.ts              # TypeScript types
│       └── router/                     # Vue Router
├── exemplos/                           # Sample XML files for testing
├── Makefile
├── docker-compose.yml
├── .env.example
└── README.md
...
- **Raw XML stored in PostgreSQL** — `nfe_uploads` table holds the original XML as `BYTEA`. The queue carries only the `upload_id`, keeping RabbitMQ lightweight and the data durable in the database.
...
- **Quarantine** — NF-es that fail XSD validation, mod 11 checks, or parsing are isolated in a separate view and automatically deleted after a configurable retention period. This prevents corrupted or invalid uploads from polluting the main dashboard.
- **Mod 11 validation** — Both the NF-e access key (SEFAZ MOC section 5.4, weights 2-9) and the issuer CNPJ (Receita Federal, weights 5,4,3,2,9,8,7,6,5,4,3,2) are validated using the correct mod 11 algorithms before persistence.
- **XSD structural validation only** — Cryptographic signature verification (XMLDSig) requires C14N canonicalization, which has no mature native support in Go. The structural integrity of the fiscal document is validated; signature authenticity is documented as a known limitation.
...
used for structural validation of uploaded XML files. Only authorized NF-e documents in
`<nfeProc>` format are accepted.
